...
> 🔺 **iOS 26 / 27（含 beta5、beta6）必看**：Apple 从 iOS 26 开始大幅强化了 `locationd` 的定位**缓存**，系统会把真实定位缓存到内存并长时间复用。**即使脚本已成功改写了 WLOC 响应（日志显示已修改），系统仍可能继续用旧坐标，看起来"没生效"**。
> **解决方法：重启设备**（重启会清空 `locationd` 内存缓存）。开/关飞行模式、关闭定位服务在 iOS 26+ 上**无法**清除这个缓存，必须重启。iOS 15~18 通常关开定位即可生效。

> **首次在 beta5/beta6 上从"旧配置"升级**：如果之前没效果，重新订阅一次模块（确保拿到下面这 5 个域名），并确认 HTTPS 解密列表里包含完整域名集合，然后按上面的方法重启一次设备。
